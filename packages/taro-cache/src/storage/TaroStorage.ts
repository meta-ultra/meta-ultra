import type { CacheItem, Storage } from "@meta-ultra/cache/dist/cjs/cache.core";
...
class TaroStorage implements Storage {
  ...
  async initialize(id: string): Promise<Map<string, CacheItem>> {
    ...
    let item = null;
    // throw an error messaged with "getStorage:fail not found", when there is no entry matched with the key.
    try {
      const { data } = await getStorage({ key: id });
      item = data;
    } catch (e) {
      // ignore the error
    }
